...
#Statement: I want to see how the weight of all the snowshoe hares both female and male will vary during the study 
```
...
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#Snowshore hares, otherwise known as Lepus americanus, are a keystone prey species that play an ecologically important role in northern boreal forests and experience population fluctuations of 8-11 years. This capture-recapture study of snowshoe hares was conducted at 5 locales in the Tanana valley from study sites Tok in the east to Clear in the west from 1999-2012 with a time duration of 13 years. Snowshoe hare population densities were the highest in 1999 and declined thereafter. This study did not find any declines in apparent survival during declining densities in these study populations (Source: Kielland K., F. S. Chapin, R. W. Ruess. 2017. Snowshoe hare physical data in Bonanza Creek Experimental Forest: 1999-Present. Environmental Data Initiative. https://doi.org/10.6073/pasta/03dce4856d79b91557d8e6ce2cbcdc14.)
```
...
snow_plot <- ggplot(snow_tidy, aes(x=weight, color=sex, fill=sex)) +
  ...
  labs(title="Weight Distribution of Female and Male Snowshoe Hares from 1999 - 2012",x="Weight (g)", y = "Frequency Count of Snowshoe Hares", caption = "Weight distribution of male and female snowshoe hares (Lepus americanus) \n in Bonanza Creek Experimental Forest (1999-2012) \n (Source: Kielland K., F. S. Chapin, R. W. Ruess. 2017. Snowshoe hare physical data in Bonanza Creek Experimental Forest: 1999-Present. Environmental Data Initiative. https://doi.org/10.6073/pasta/03dce4856d79b91557d8e6ce2cbcdc14.))") + 
  theme_bw()

snow_plot+theme(plot.caption=element_text(hjust=0.5)) #Adjusting the alignment of the title as well as the citation 

```

### Interpreting the Bonanaza Creek snowshoe hare population graph 
```{r, echo = TRUE, message = FALSE, warning = FALSE} 
...
### Creating a new dataframe with desirable traits   
```{r, echo = TRUE, message = FALSE, warning = FALSE, results = "hide"}
...
### Calculating  descriptive statistics and creating a new data frame 
```{r, echo = TRUE, message = FALSE, warning = FALSE, results = "hide"} 
...
#Creating a new data frame that encompasses all of the previous descrptive statistical results of the weight for female snowshoe hares 
snow_tbl <- data.frame(
...
```{r, echo = TRUE, message = FALSE, warning = FALSE}
#Creating a kable table with all of the previous descriptive statistical results of the weight for female snowshoe hares 
...
### Interpreting the finalized HTML data table 
```{r, echo = TRUE, message = FALSE, warning = FALSE}  
...
## HW2: Principal Components Analysis (PCA) of USDA Nutrient Food Groups 
...
### Farmers and the USDA helping each other out 
```{r echo=FALSE, out.width='100%', fig.align = "center", echo = FALSE}
knitr::include_graphics("images/usda.jpg")
```

### Coming up with PCA analysis and variance determination 
```{r, message = FALSE, warning = FALSE, results = "hide"}

#Coming with PCA analysis 
usda_pca <- prcomp(nutrients[9:30], scale = TRUE)

#Determining how much variance is in each PCA 
summary(usda_pca)
```

### Creating the finalized PCA analysis biplot 
```{r, PCA, biplot, message = FALSE, warning = FALSE}
#Coming up with the biplot graph for the dataset and using autoplot since the original dataset is a mess 
...
#Changing the alignment of both title and caption for th PCA biplot 
usda_bilplot + theme(plot.caption=element_text(hjust=0.5)) + theme(plot.title=element_text(hjust = 0.5))
```

### Interpreting the PCA Biplot: Fruits and Vegetable Nutrient Levels Example 
